# Number of patients waiting more than 52 weeks - Dec 2025 (with DTA)
...
# read in all the monthly summary files 
...
# combine monthly data and add month row
...
# sums together columns over 52 weeks and ensures each row
# is a unique combination of region and month (now stored in over52_by_region)
cols_over52 <- grep("^greater5[2-9]|^greater[6-9][0-9]|^greater1[0-9]{2}|^greater104", names(monthly_data))
monthly_data$over52 <- rowSums(monthly_data[, cols_over52])
over52_by_region <- monthly_data %>%
  group_by(region_name, month) %>%
  summarise(over52 = sum(over52))

# set a nice order of regions for clearer plotting
over52_by_region$region_name <- factor(
  over52_by_region$region_name, 
  levels = c(
...
# exclude NHS ENGLAND from the data
over52_by_region <- over52_by_region %>%
  filter(region_name != "NHS ENGLAND")

##### GRAPH DATA #####
p1 <- ggplot(over52_by_region, aes(x = month, y = over52, group = region_name, color = region_name)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
